...
class UpdateTest extends TestCase
{
    ...
    public function test_if_team_can_be_updated_with_valid_name()
    {
        ...
        $this->put('/api/team/' . $team['id'], ["name" => "Barca"])->assertStatus(200);
    ...
    {
        $team = $this->post('/api/team', ["name" => "Juventus"])->assertStatus(201);
        $this->put('/api/team/' . $team['id'], ["name" => str_repeat("X", 256)])->assertStatus(302);
        $team = Team::find($team['id']);
    ...
    {
        $team = $this->post('/api/team', ["name" => "Juventus"])->assertStatus(201);
        $this->put('/api/team/' . $team['id'])->assertStatus(302);
        $team = Team::find($team['id']);
        $this->assertEquals("Juventus", $team->name);
    }
}
